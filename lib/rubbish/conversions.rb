module Blix
  ...
  # recursively convert a structure of hashes to local class objects
  class ParseException < StandardError;end
  
  def self.convert_to_class( hash)
    if hash.kind_of? Hash
      ...
      if klass == 'value'
        ...
        klass = klass[0..-4]
        raise ParseException,"#{klass} is not Valid!" unless klass_info = VALID_KLASS[klass.downcase.to_sym]
